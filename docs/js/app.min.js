...
// }
$scope.toggleGradeLevelChart=function(){$scope.showGradeLevelChart=!$scope.showGradeLevelChart},$scope.newAssessment=function(){$scope.assessment={Grade:$scope.student.Grade,Student:[$scope.student],Date:new Date,Type:"Fiction",Accuracy:"",Comprehension:"",Fluency:"",Mastery:"Hard",Closeness:"",Notes:""}},$scope.clickAssessment=function(item,e){if($scope.edit)if($scope.assessment&&item.id==$scope.assessment.id)$scope.assessment=null;else for(var j=0;j<$scope.assessments.length;j++)if($scope.assessments[j].id==item.id){$scope.assessments[j].Date=new Date($scope.assessments[j].Date),$scope.assessment=$scope.assessments[j];for(var i=0;i<$scope.colors.length;i++)$scope.colors[i].id==$scope.assessment.Color[0]&&$scope.setColor($scope.colors[i])}},$scope.updateCloseness=function(){if($scope.growth.length&&$scope.assessment.TextLevel&&$scope.assessment.Date){
// Get % of school-year that is complete
// Note: months are -1 what you expect so 7 = August, 5 = June
var now=new Date($scope.assessment.Date),start=new Date(now.getFullYear(),7,14);start=start>now?new Date(now.getFullYear()-1,7,14):start;for(var end=new Date(start.getFullYear()+1,5,14),fraction=(now-start)/(end-start),gradeLevels=[],gradeKey=null,i=0;i<$scope.growth.length;i++)$scope.growth[i].GradeLevel==$scope.student.Grade&&(gradeKey=gradeKey?gradeKey:i,gradeLevels.push($scope.growth[i]));var key=Math.round(fraction*gradeLevels.length/gradeLevels.length);console.log("key",key,gradeKey);var expected=0,current=0,foundTextLevel=!1;for($scope.growthLevel={current:$scope.assessment.TextLevel},console.log($scope.growth),i=0;i<$scope.growth.length;i++){var item=$scope.growth[i];i<gradeKey+key&&(console.log(i,item.Growth),expected+=parseFloat(item.Growth),$scope.growthLevel.expected=item.TextLevel),foundTextLevel||(current+=parseFloat(item.Growth)),item.TextLevel==$scope.assessment.TextLevel&&(foundTextLevel=!0)}$scope.assessment.Closeness=current-expected,$scope.assessment.GradeLevel=Math.floor(current),console.log("expected",expected,current),console.log(gradeLevels),
//
// var gradeLevels = [];
// var expected = 0;
// var current = 0;
