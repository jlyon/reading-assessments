...
//***************************************
angular.module("app",["ui.router","ngAnimate","chart.js","firebase"]).run(["$sce","$timeout","$rootScope","$state","$stateParams","$window","$firebaseAuth",function($sce,$timeout,$rootScope,$state,$stateParams,$window,$firebaseAuth){
// It's very handy to add references to $state and $stateParams to the
// $rootScope
$rootScope.$state=$state,$rootScope.$stateParams=$stateParams,$rootScope.cache={},$rootScope.config={years:[{label:"2018-19",base:"appV6NqAqUOyQvJUW",key:"keyNIbNk17BU31gT8",table:"tblPHmjv1JnCHTSkV"},{label:"2017-18",base:"appA1AUk2jnO8Xmzt",key:"keyNIbNk17BU31gT8",table:"tbluC9NfD8WSZlvZs"}],admins:{"jeff@albatrossdigital.com":{role:"admin"},"lisa.perloff@lighthousecharter.org":{role:"admin",teacher:"lisa"},"virginia.mcmanus@lighthousecharter.org":{role:"admin",teacher:"virginia"},"ana.garcia@lighthousecharter.or":{role:"admin",teacher:"ana"},"daelana.burrell@lighthousecharter.org":{role:"admin"},"tiffany.do@lighthousecharter.org":{role:"admin"},"maricruz.martinez@lighthousecharter.org":{role:"admin"},"robbie.torney@lighthousecharter.org":{role:"admin"}},default_year:"2018-19"},
// Handle year changing
$rootScope.activeYear=$window.localStorage.getItem("readingAssessmentYear")?JSON.parse($window.localStorage.getItem("readingAssessmentYear")):$rootScope.config.years[0],$rootScope.setYear=function(year,e){e.preventDefault(),$window.localStorage.setItem("readingAssessmentYear",JSON.stringify(year)),$rootScope.activeYear=year,$state.go("students",{reload:!0}),$window.location.href="/?"+(new Date).getTime()+"#/admin/students"},
// Authentication
...
// Airtable
var Airtable=require("airtable");$rootScope.Airtable=new Airtable({apiKey:$rootScope.activeYear.key}).base($rootScope.activeYear.base);
...
$rootScope.getNumericalReadingLevel=function(textLevel){for(var num=parseFloat($rootScope.growth[0].GradeLevel),i=0;i<$rootScope.growth.length&&$rootScope.growth[i].TextLevel!=textLevel;i++)num+=parseFloat($rootScope.growth[i].Growth);return num},
...
var now=date,start=new Date(now.getFullYear(),7,10);start=start>now?new Date(now.getFullYear()-1,7,10):start;var end=new Date(start.getFullYear()+1,5,14),fraction=(now-start)/(end-start);console.log(now,start,end);for(var gradeLevels=[],gradeKey=null,i=0;i<$rootScope.growth.length;i++)$rootScope.growth[i].GradeLevel==Math.floor(gradeLevel)&&(gradeKey=gradeKey?gradeKey:i,gradeLevels.push($rootScope.growth[i]));var key=Math.round(fraction*gradeLevels.length);
...
$scope.language="en",$scope.setLanguage=function(language){$scope.language=language},$scope.t=function(str){var translations={en:{},es:{"Reading Assessments":"Exámenes de lectura","At a Glance":"A la vista",Quarter:"Cuarto",Level:"Nivel",Expected:"Nivel esperado","Grade Band":"Grado",Growth:"Crecimiento",Goal:"Meta","Reading Progress":"Progreso de lectura","All Assessments":"Exámenes"}};return void 0!=translations[$scope.language][str]?translations[$scope.language][str]:str},$scope.show=!1,$scope.showAllAssessments=$scope.edit,$scope.showChart=!0,$scope.showEditGoal=!1,$timeout(function(){$scope.link=window.location.href.replace("/admin","")},0),void 0==$rootScope.cache.GrowthCalculator){var items1=[];$rootScope.Airtable("GrowthCalculator").select({sort:[{field:"ID",direction:"asc"}]}).eachPage(function(records,fetchNextPage1){records.forEach(function(record){record.fields.id=record.id,items1.push(record.fields)}),fetchNextPage1()},function(error){$rootScope.cache.GrowthCalculator=items1,$scope.growth=items1,$scope.$apply()})}else $scope.growth=$rootScope.cache.GrowthCalculator;var calculateQuarters=function(assessment,assessments){for(var quarters=$scope.quarters,last=0,i=0;i<quarters.length;i++){
// This is a hack to support the last quarter showing the first Level of the next year.
// (for all grades but Kinder)
var automaticallyAdd=i==quarters.length-1&&0!=$scope.student.Grade?1:0;
