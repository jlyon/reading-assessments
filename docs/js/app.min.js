...
// Add assessments
for(var i=0;i<$scope.assessments.length;i++)data.addRow([new Date($scope.assessments[i].Date),getNumericalReadingLevel($scope.assessments[i].TextLevel),null]);var options={
// vAxis: {
//   title: 'Reading Level'
// },
series:{1:{curveType:"function"},2:{curveType:"function"}},chartArea:{left:30,top:10,width:"80%",height:"90%"},legend:{position:"top right"}},chart=new google.visualization.LineChart(document.getElementById("chart_div"));chart.draw(data,options)}$scope.assessments&&$scope.quarters&&(google.charts.load("current",{packages:["corechart","line"]}),google.charts.setOnLoadCallback(drawCurveTypes))}function getNumericalReadingLevel(textLevel){for(var num=0,i=0;i<$scope.growth.length&&$scope.growth[i].TextLevel!=textLevel;i++)num+=parseFloat($scope.growth[i].Growth);return num}function getExpectedReadingLevel(date,mastery){
