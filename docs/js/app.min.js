...
$rootScope.auth.$onAuthStateChanged(function(firebaseUser){if(firebaseUser=JSON.parse(JSON.stringify(firebaseUser))){var role;role=firebaseUser&&null!=firebaseUser&&firebaseUser.email&&$rootScope.config.admins.indexOf(firebaseUser.email)!==-1?"admin":"student",firebaseUser.role=role,firebaseUser.time=new Date,firebaseUser.providerData=void 0}else role=null;$rootScope.firebaseUser!=firebaseUser&&($rootScope.firebaseUser=firebaseUser,$window.localStorage.setItem("firebaseUser",JSON.stringify(firebaseUser))),firebaseUser&&"login"===$state.current.name&&("admin"===role?$state.go("students"):$state.go("myFlashcards"))});
...
// Check login
$rootScope.$on("$stateChangeSuccess",function(event,toState,toParams,fromState,fromParams){toState.auth&&($rootScope.firebaseUser||(event.preventDefault(),$state.go("login",{msg:"You need to login"})),toState.auth&&toState.auth!=$rootScope.firebaseUser.role&&(event.preventDefault(),$state.go("login",{msg:"Sorry, you don't have access."}))),resetDropdown()}),$rootScope.logout=function(e){$rootScope.firebaseUser=null,$window.localStorage.setItem("firebaseUser",null),$rootScope.auth.$signOut()};
// Airtable
...
//////////////////////////
$stateProvider.state("login",{url:"/?msg",templateUrl:"views/login.html",controller:function($scope,$rootScope,$state,$filter,$timeout){$scope.clickLogin=function(e){e.preventDefault(),console.log($state),$scope.msg=$state.params.msg}}}).state("students",{url:"/admin/students/:query",templateUrl:"views/students.html",
// auth: true,
/*resolve: {
