...
// $rootScope
$rootScope.$state=$state,$rootScope.$stateParams=$stateParams,$rootScope.cache={};var Airtable=require("airtable");$rootScope.Airtable=new Airtable({apiKey:"keyNIbNk17BU31gT8"}).base("appA1AUk2jnO8Xmzt");
...
var now=new Date(date),start=new Date(now.getFullYear(),7,14);start=start>now?new Date(now.getFullYear()-1,7,14):start;for(var end=new Date(start.getFullYear()+1,5,14),fraction=(now-start)/(end-start),gradeLevels=[],gradeKey=null,i=0;i<$rootScope.growth.length;i++)$rootScope.growth[i].GradeLevel==gradeLevel&&(gradeKey=gradeKey?gradeKey:i,gradeLevels.push($rootScope.growth[i]));var key=Math.round(fraction*gradeLevels.length);
// If Mastery is Hard they are really one step back
// @todo: is this correct?
void 0!=mastery&&"Hard"===mastery&&key--,console.log("fraction",fraction);
